...
export function makeActionCreatorWithReducerWithPrefix(actionName, reducer) {
  return reducerName => makeActionCreatorWithReducer(JSON.stringify({
    reducerName,
    actionName
  }), reducer);
}
...
export function makeSimpleReducer(reducerName, initialState, otherActions) {
  const actions = mapValues(initialState, (propertyFromState, key) => {
    const creatorReducer = makeActionCreatorWithReducerWithPrefix(`UPDATE_${key}`, (state, newValue) => {
      return assign({}, state, {
        [key]: newValue
      });
    });
  ...
  return {
    actions: assign({}, getCreators(assign({}, actions, {
      setAll,
      set,
      reset
    }, createdOtherActions)), {
    ...
    reducer: createReducer(initialState, assign({}, actions, {
      resetAllActionCreatorReducer,
      setAll,
      set,
      reset
    }, createdOtherActions))
