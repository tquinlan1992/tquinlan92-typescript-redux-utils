...
export function makeActionCreatorWithReducerWithPrefix(actionName, reducer) {
  return (reducerName) =>
    makeActionCreatorWithReducer(
      JSON.stringify({ reducerName, actionName }),
      reducer
    );
}
...
export function makeSimpleReducer(reducerName, initialState, otherActions) {
  const actions = mapValues(initialState, (propertyFromState, key) => {
    const creatorReducer = makeActionCreatorWithReducerWithPrefix(
      `UPDATE_${key}`,
      (state, newValue) => {
        return assign({}, state, { [key]: newValue });
      }
    );
    return creatorReducer(reducerName);
  });

  const reset = makeActionCreatorWithReducer(
    getActionName(reducerName, "Reset"),
    (state, keysToReset) => {
      return assign({}, state, pick(initialState, keysToReset));
    }
  );
  const resetAllActionCreatorReducer = makeActionCreatorWithReducer(
    getActionName(reducerName, "RESET_ALL"),
    () => {
      return initialState;
    }
  );
  const resetAll = () => resetAllActionCreatorReducer.actionCreator(null);
  const setAll = makeActionCreatorWithReducer(
    getActionName(reducerName, "SET_ALL"),
    (state, newValue) => {
      return newValue;
    }
  );
  const set = makeActionCreatorWithReducer(
    getActionName(reducerName, "SET"),
    (state, newStateValues) => {
      return assign({}, state, newStateValues);
    }
  );
  const createdOtherActions = mapValues(otherActions, (reducer, key) => {
    return makeActionCreatorWithReducer(
      getActionName(reducerName, key),
      reducer,
      true
    );
  });
  return {
    actions: assign(
      {},
      getCreators(
        assign({}, actions, { setAll, set, reset }, createdOtherActions)
      ),
      { resetAll }
    ),
    reducer: createReducer(
      ...
      assign(
        {},
        actions,
        { resetAllActionCreatorReducer, setAll, set, reset },
        createdOtherActions
      )
    ),
  };
