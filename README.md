...
const getResultsSelector = createSelector(selectors.state1.results, mapResults);
...
```ts
function createConnectProps<AppState>(): <
  MapStateToProps extends (state: AppState, ownProps: any) => {},
...
>(
  ...
  Connected: import("react-redux").ConnectedComponentClass<
    ...
    >,
    Pick<any, never> & Parameters<MapStateToProps>[1]
  >;
};
```
